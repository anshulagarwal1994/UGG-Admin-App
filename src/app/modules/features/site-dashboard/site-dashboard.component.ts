...
export class SiteDashboardComponent implements OnInit {
  ...
  constructor(private readonly formBuilder: FormBuilder, private indexedDBService: IndexedDBService,
    private httpDataService: HttpDataService, public filterService: GridFilterService,
    private router: Router, private translate: TranslateService,
    public translateConfigService: TranslateConfigService, private popUpService: PopUpService,
  ...
  displayedColumns: string[] = ['chargePointId', 'name', 'location', 'status', 'connector', 'connector1'];
  ...
  getSiteNameAutoComplete() {
    this.siteNameValues = this.filterService.getFilterObject(this.dataSource.data, 'name');
  ...
  getLocationAutoComplete() {
    this.locationValues = this.filterService.getFilterObject(this.dataSource.data, 'location');
  ...
  getChargePointAutoComplete() {
    this.chargePointValues = this.filterService.getFilterObject(this.dataSource.data, 'chargePointId');
  ...
  getUserValid() {
    ...
    return this.httpDataService.get(AppConstants.APIUrlUserValidatetUrl + ids).subscribe((res: any) => {
    ...
    return this.httpDataService.get(AppConstants.APIUrlTenantNameListtUrl).subscribe((res: Tenant[]) => {
      ...
      setTimeout(() => {
        if (this.tenants.length) {
          ...
          localStorage.setItem('selectedTenant', this.tenants[0].tenantId.toString());
          this.routerExtService.setRouteValue(AppConstants.TenantID, this.tenants[0].tenantId?.toString());
  ...
  getSiteDashboard(tenantId: any) {
    return this.httpDataService.getById(AppConstants.APIUrlSiteDashboardtUrl, tenantId).subscribe((res) => {
  ...
  siteUpdate(siteData: any) {
    ...
    this.routerExtService.setRouteValue(AppConstants.SiteID, siteData.siteId.toString());
  ...
  getSites(tenantId: Guid) {
    ...
    this.httpDataService.get(AppConstants.APIUrlSiteListUrl + tenantId + '/' + this.deletedRecords).subscribe((res: SiteList[]) => {
      // res.filter((s: any) => {
      this.siteList = [];
      this.mapList = [];
      ...
      res.forEach((currentSite: any, currentSiteIndex: any) => {
        ...
        if (currentSite?.chargePointList?.length > 0) {
          currentSite?.chargePointList?.forEach((currentChargePoint: any, chargePointIndex: any) => {
            let siteObj = new SiteList();
            let mapObj = new SiteList();
            duplicateSite = this.mapList.filter(s => s.address.street == currentSite.address?.street);
            ...
            if (duplicateSite.length > 0) {
              mapObj.noOfChargePoints = duplicateSite[0].noOfChargePoints + currentSite?.chargePointList?.length;
              // console.log(mapObj.noOfChargePoints);
            }
            else
              mapObj.noOfChargePoints = currentSite?.chargePointList?.length;
            ...
            siteObj.location = currentSite.address.city + ', ' + currentSite.address.state + ', ' + currentSite.address.country;
            ...
            siteObj.numberOfConnectors = currentChargePoint.numberOfConnectors;

            currentChargePoint?.connectors?.forEach((currentConnector: any, connectorIndex: any) => {

              if (connectorIndex > 0 && connectorIndex < 3) {
                // Calculate the connector count base on the status, this will be used in MAP Tooltip
                if (currentConnector.status == AppConstants.Occupied && currentChargePoint.status == AppConstants.Online) {
                  occupiedConnectors++;
                }
                else if (currentConnector.status == AppConstants.Faulted && currentChargePoint.status == AppConstants.Online) {
                  faultedConnectors++;
                }
                else if (currentConnector.status == AppConstants.Available && currentChargePoint.status == AppConstants.Online) {
                  availableConnectors++;
                }
                else if (currentConnector.status == AppConstants.Offline && currentChargePoint.status == AppConstants.Online) {
                  offlineConnectors++;
                }
                else {
              ...
              if (connectorIndex == 1) {
                totalConnectors++;
                siteObj.connector1Name = currentConnector.meter != null ? currentConnector.meter : null;
                siteObj.connector1Status = currentConnector.status;
              }
              if (connectorIndex == 2) {
                totalConnectors++;
                siteObj.connector2Name = currentConnector.meter != null ? currentConnector.meter : null;
  ...
  filterChange(filter: any, event: any) {
    if (event.option != undefined) {
      ...
      if (!Helper.isNullOrEmpty(this.siteNameControl.value))
        ...
        this.filterValues[filter?.columnProp] = this.chargePointControl.value.toString().trim().toLowerCase();
    }
